...
export default function HospitalInventoryTab() {
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      setRegisteredVaccines(
        Array.isArray(formulary) ? formulary.map((f) => f.vaccineName) : []
      );
  ...
  const handleLogWastage = async ({ vaccineId, quantity, reason, reportedBy, notes, incidentDate }) => {
    try {
      await inventoryService.logWastage(vaccineId, {
        quantity,
        reason,
        reportedBy,
        notes,
    ...
    <div className="hospital-manage-appointments-wrapper">
      <div className="hospital-manage-appointments-card" style={{ maxWidth: '1240px' }}>
        ...
        <div className="inventory-toolbar">
          ...
          <div className="inventory-filter-pills">
            <div className="filter-pill-group">
              ...
              <button type="button" className={`filter-pill pill-alert ${statusFilter === 'low' ? 'active' : ''}`} onClick={() => setStatusFilter('low')}>
                ⚠️ Low Stock ({lowStockCount})
              </button>
              <button type="button" className={`filter-pill ${statusFilter === 'sufficient' ? 'active' : ''}`} onClick={() => setStatusFilter('sufficient')}>
                ✓ Healthy
              </button>
              <button type="button" className={`filter-pill pill-warning ${statusFilter === 'expiring' ? 'active' : ''}`} onClick={() => setStatusFilter('expiring')}>
                ⏳ Expiring Soon ({expiringCount})
              </button>
              <button type="button" className={`filter-pill ${statusFilter === 'ultracold' ? 'active' : ''}`} onClick={() => setStatusFilter('ultracold')}>
                ❄️ Ultra-Cold (-70°C)
              </button>
            </div>
        ...
        <div className="inventory-footer-notice">
          ...
          <button type="button" className="btn-sync-registry" onClick={() => showToast('Cold chain registry status: All batches verified in consensus.')}>
